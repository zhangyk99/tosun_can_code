...
//    char serial[100]{};
//    char dvc_name[100]{};
    char *pManufacturer = nullptr;
    char *pProduct = nullptr;
    char *pSerial = nullptr;
    char *pDvc_name = nullptr;
    int32_t dvc_type;
    int32_t canCount;
    bool isFD;
    int32_t linCount;
    ...
    std::cout << "get device information, result: " << tscan_get_device_info(0, &pManufacturer, &pProduct, &pSerial) << std::endl;
...
//    std::cout << "can counts: " << canCount << " is FD: " << isFD << " lin count: " << linCount << " flexray count: " << frCount << " ether count: " << ethCount << std::endl;
    ...
    std::cout << "get device information, result: " << tscan_get_device_info(1, &pManufacturer, &pProduct, &pSerial) << std::endl;
...
//        std::cout << "set can frame, result: " << tscan_transmit_can_sync(handle1, &canFrame, 100) << std::endl;
    ...
    std::cout << "device disconnect, result: " << tscan_disconnect_by_handle(handle0) << std::endl;
    std::cout << "device disconnect, result: " << tscan_disconnect_by_handle(handle1) << std::endl;
}